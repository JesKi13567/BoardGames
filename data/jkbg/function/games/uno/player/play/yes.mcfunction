...
function jkbg:games/uno/player/play/yes_ with storage jkbg:uno cards.temp
...
function jkbg:games/uno/player/play/yes_loop

# 修改列表
execute store result storage jkbg:uno cards.temp.mem int 1 run scoreboard players get @s jkbg.uno.order
function jkbg:games/uno/player/play/yes__ with storage jkbg:uno cards.temp

# 恢复
scoreboard players set @s[scores={jkbg.uno.color=1..4}] jkbg.uno.color 0

# 刷新
function jkbg:games/uno/player/refresh

# 检测是否打完卡牌
execute if score @s jkbg.uno.cards matches 1 run say UNO
execute if score @s jkbg.uno.cards matches 0 run function jkbg:games/uno/player/play/over
